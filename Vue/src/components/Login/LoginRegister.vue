    ...
    <div class="content_body">
        ...
        <div class="register_content" v-else>
            ...
            <div class="clicked">
                ...
                <span
                >已阅读并同意账号<a href="javascript:;">用户协议</a>和
                <a href="javascript:;">隐私政策</a>
                </span>
            </div>
            </form>
...
import {activeRouter} from '@/router/index.js';
export default {
    name:'LoginRegister',
    data() {
        return {
    ...
    methods: {
        ...
        GetLogin(){
            // 1.首先判断账号密码是否符合规范
            if(this.username==''||this.password==''){alert("账号或密码为空，请输入")}
            else{
                if(!this.isChecked){
                    alert("请同意该条款");
                }else{
                    const unameIsValid = this.unameValidateCredentials(this.username);
                    const psdIsValid = this.pwdValidataCredentials(this.password);
                    if(!unameIsValid){
                        alert("用户名长度至少为6，且只能包含字母、数字、下划线");
                    }else if(!psdIsValid){
                        alert("密码长度至少为 8，包含至少一个大写字母、一个小写字母、一个数字和一个特殊字符");
                    }else{
                        //如果符合规范，发送请求
                        request.post('/login', {
        ...
        unameValidateCredentials(username){
            const usernameRegex = /^[a-zA-Z0-9_]{6,}$/;
            return usernameRegex.test(username);
        },
        pwdValidataCredentials(password){
...
.content_body {
    ...
    .login_content,.register_content{
        ...
        .login_btn,.register_btn{
            ...
            cursor: pointer;
            &:hover{
                background-color: rgb(15, 234, 62);
            }
        }
        .forget {
            width: 80%;
